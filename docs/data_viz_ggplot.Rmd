...
```{r anscombe.clean, message=F, warning=F}
anscombe <- anscombe |> 
...
```{r anscombe.summary, message=F, warning=F}
anscombe |> 
  group_by(set) |> 
...
```{r anscombe.set1.fit.named_coef_indexing}
(intercept <- coef(fit)["(Intercept)"])
(slopex <- coef(fit)["x"])
```
:::

```{r anscombe.set1.plot.regress}
ggplot(data = set1_data, aes(x = x, y = y))+
  geom_point()+
...
```{r starwars.data, message=F, warning=F}
data("starwars")
```
...
```{r starwars.clean, message=F, warning=F}
...
```{r swhuman.plot.point_layer_color}
ggplot(data = sw_human_data, aes(x = height, y = mass)) + 
...
```{r swhuman.plot.scale_color_manual}
ggplot(data = sw_human_data, aes(x = height, y = mass, color = eye_color)) + 
  geom_point(size = 5) +
...
```{r swhuman.plot.scale_color_brewer}
ggplot(data = sw_human_data, aes(x = height, y = mass, color = eye_color)) + 
  geom_point(size = 5) +
...
```{r starwars.plot.xylim_coord_cartesian_regress}
ggplot(data = starwars, aes(x = height, y = mass)) + 
  geom_point(size = 2) +
  scale_colour_brewer(palette = "RdYlBu") +
...
```{r swhuman.plot.boxplot_coordcart}
ggplot(data = sw_human_data, aes(x = gender, y = height)) +
  geom_boxplot() +
...
```{r swhuman.plot.group_color_home}
ggplot(data = sw_human_data, aes(x = gender, y = height)) +
  geom_violin() +
  geom_boxplot(width = 0.25) +
...
```{r swhuman.plot.color_group_mass}
ggplot(data = sw_human_data, aes(x = gender, y = height)) +
  geom_violin() +
  geom_boxplot(width = 0.25) +
...
```{r simdat.plot.boxplot.as_factor}
...
```{r simdat.plot.boxplot.connected_point}
ggplot(data = data, aes(x = as.factor(week), y = measure)) +
  geom_boxplot() +
  geom_point() +
...
```{r penguins.data.head}
data("penguins")
head(penguins)
```

```{r penguins.data.glimpse}
glimpse(penguins)
```

```{r penguins.data.naomit}
