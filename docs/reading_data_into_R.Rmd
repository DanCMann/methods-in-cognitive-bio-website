...
write.table(data1, file = "data1.txt", row.names = F)
```

Now lets read the csv file back into R
```{r}
data1_csv <- read.csv("data1.csv")
```
...
```{r}
data1_txt <- read.table("data1.txt")
head(data1_txt)
```
...
```{r}
data1_txt <- read.table("data1.txt", header = T)
head(data1_txt)
```
```{r}
str(data1_txt)
...
data2_csv <-read.csv("data2.csv", header = T)
```
...
```{r}
data2_csv <-read.csv("data2.csv", header = T, stringsAsFactors = F)
new_row <- c("Lady", 6)
